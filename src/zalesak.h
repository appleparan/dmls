...
class zalesak
{
        static void make_sphere(eufield::grid *grid, float* phi, geometry::vector pos, float radius)
        {
            for (int i = 0; i < grid->nx; ++i)
                for (int j = 0; j < grid->ny; ++j)
                {
                    ...
                    {
                        if (fabs(val2) > fabs(y - bottom_wall)) val2 = y - bottom_wall;
                        if (fabs(val2) > fabs(y - top_wall)) val2 = top_wall - y;
                    }
                    else if (bottom_wall <= y && y <= top_wall)
                    {
                        if (fabs(val2) > fabs(x - left_wall)) val2 = x - left_wall;
                        if (fabs(val2) > fabs(x - right_wall)) val2 = right_wall - x;
                    }
                    else
                    {
        ...
        static void init_domain(eufield::grid *&grid, eufield::domain *&dom, lfield::dimarkerset *&dms, float omega)
        {
            ...
            for (int i = 0, k = 0; i < nx; ++i)
                ...
                {
                    grid->idxs[i][j] = k++;
                    if (i == 0 || i == nx - 1 || j == 0 || j == ny - 1) grid->bcs[i][j] = 1;
                }
