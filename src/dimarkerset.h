...
class dimarkerset: public std::list<dimarker>
{
	...
	{
		if (phi1 * phi2 > 0 || (fabs(phi1) < 1e-6 && fabs(phi2) < 1e-6))
			return false;
	...
	void insert_markers(geometry::vector sec1, geometry::vector sec2, int current_count)
	{
		...
		if (result.getphi(domain->grid->getpos(i, j)) * phi < 0.)
			result.normal *= -1;
...
public:
	...
	void init_markers_delta()
	{
		...
		for (int i = 0; i < domain->grid->nx; ++i)
			for (int j = 0; j < domain->grid->ny; ++j)
			{
				...
				if (!(domain->phi[idx] * domain->phi[idxi] <= 0 || domain->phi[idxi] * domain->phi[idxij] <= 0 || domain->phi[idxij] * domain->phi[idxj] <= 0 || domain->phi[idxj] * domain->phi[idx]
						<= 0))
					continue;
	...
	void init_markers_intersects()
	{
		for (int i = 0; i < domain->grid->nx; ++i)
			for (int j = 0; j < domain->grid->ny; ++j)
			{
				...
				if (!(sectedB || sectedR || sectedT || sectedL) || (sectedB && sectedR && sectedT && sectedL))
					continue;

				if (sectedB && sectedR)
					insert_markers(sectB, sectR, 0);
				if (sectedR && sectedT)
					insert_markers(sectR, sectT, 0);
				if (sectedT && sectedL)
					insert_markers(sectT, sectL, 0);
				if (sectedL && sectedB)
					insert_markers(sectL, sectB, 0);

				if (sectedL && sectedR && !(sectedT || sectedB))
					insert_markers(sectL, sectR, 0);
				if (sectedT && sectedB && !(sectedL || sectedR))
					insert_markers(sectT, sectB, 0);
	...
	void add_markers()
	{
		...
		for (int i = 0; i < domain->grid->nx; ++i)
			for (int j = 0; j < domain->grid->ny; ++j)
			{
				...
				if (sectedB && sectedR)
					insert_markers(sectB, sectR, c);
				if (sectedR && sectedT)
					insert_markers(sectR, sectT, c);
				if (sectedT && sectedL)
					insert_markers(sectT, sectL, c);
				if (sectedL && sectedB)
					insert_markers(sectL, sectB, c);

				if (sectedL && sectedR && !(sectedT || sectedB))
					insert_markers(sectL, sectR, c);
				if (sectedT && sectedB && !(sectedL || sectedR))
					insert_markers(sectT, sectB, c);
	...
	void delete_bad_vecs(std::list<iterator>* lst)
	{
		...
		for (std::list<iterator>::iterator i = lst->begin(); i != lst->end(); ++i)
		{
			...
			for (std::list<iterator>::iterator j = i; j != lst->end(); ++j)
			{
				...
				if (geometry::dot((*i)->pos, (*j)->pos) > 0.9)
					ok[ii] = ok[jj] = true;
			}
			if (!ok[ii])
				del[ndel++] = i;
	...
	void delete_markers()
	{
		...
		for (int i = 0; i < domain->grid->n; ++i)
		{
			...
			if (cellmarkers[i]->size() > NMAX)
				delete_bad_vecs(cellmarkers[i]);

			if (false & cellmarkers[i]->size() > NMAX)
				delete_last_n(cellmarkers[i], NMAX - cellmarkers[i]->size());
