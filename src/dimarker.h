...
class dimarker
{
	...
	{
		if (i <= 0 || j <= 0 || i >= domain->grid->nx || j >= domain->grid->ny)
			return;
		...
		if ((stronger < 0. && phinode < phi) || (stronger > 0. && phinode > phi))
			apply_values(idx, phi, domain, correcteds, correctors);
	}
...
public:
	...
	void correct(eufield::domain* domain, bool* correcteds, dimarker** correctors)
	{
		...
		if (!can_correct(i, j, domain->phi[domain->grid->operator ()(i, j)], domain, r))
			return;
