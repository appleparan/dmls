...
class fmm
{
        ...
        void init_interface_adjacents(bool anchored)
        {
            for (int i = 0; i < grid->nx; ++i)
                for (int j = 0; j < grid->ny; ++j)
                {
                    ...
                    if (data[m.xm] < 0) xm_val = val / (val - data[m.xm]) * grid->h;
                    if (data[m.xp] < 0) xp_val = val / (val - data[m.xp]) * grid->h;
                    if (data[m.yp] < 0) yp_val = val / (val - data[m.yp]) * grid->h;
                    if (data[m.ym] < 0) ym_val = val / (val - data[m.ym]) * grid->h;

                    if (xm_val >= 0 || xp_val >= 0 || yp_val >= 0 || ym_val >= 0)
                    {
                        if (anchored) result[m.p] = data[m.p];
                        else result[m.p] = select(select(xm_val, xp_val), select(ym_val, yp_val));
                        accept(m);
                    }
                }
        }
        ...
        {
            if (conds[m.p] == alive) alive_nodes->erase(alive_iters[m.p]);
            conds[m.p] = accepted;

            if (m.xm >= 0 && data[m.xm] > 0. && conds[m.xm] != accepted) update(m.i - 1, m.j, m.xm);
            if (m.xp >= 0 && data[m.xp] > 0. && conds[m.xp] != accepted) update(m.i + 1, m.j, m.xp);
            if (m.ym >= 0 && data[m.ym] > 0. && conds[m.ym] != accepted) update(m.i, m.j - 1, m.ym);
            if (m.yp >= 0 && data[m.yp] > 0. && conds[m.yp] != accepted) update(m.i, m.j + 1, m.yp);
        }

        void update(int i, int j, int idx)
        {
            if (grid->bcs[i][j] != 0) return;
            ...
            if (conds[m.xm] == accepted) xm_val = data[m.xm];
            if (conds[m.xp] == accepted) xp_val = data[m.xp];
            if (conds[m.ym] == accepted) ym_val = data[m.ym];
            if (conds[m.yp] == accepted) yp_val = data[m.yp];
        ...
        {
            if (phi1 < 0) return calc_phi(phi2);
            if (phi2 < 0) return calc_phi(phi1);
            ...
            else return solve_parabol(2., -2. * (phi1 + phi2), phi1 * phi1 + phi2 * phi2 - h2);
